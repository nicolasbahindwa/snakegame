    ...
    return $.ajax({
      ...
      beforeSend: function (xhr) {
        ...
        return $couponCodeStatus
          .text("")
          .removeClass("alert-success alert-warning alert-error")
          .hide();
      },
    })
...
var bets = function () {
  ...
  var placeBet = function (csrfToken) {
    return $.ajax({
      ...
      url: "/bet/place",
      data: {
        guess: $(guessSelector).val(),
        wagered: $(wageredSelector).val(),
      },
      dataType: "json",
      beforeSend: function (xhr) {
        xhr.setRequestHeader("X-CSRFToken", csrfToken);
      ...
      .done(function (data, status, xhr) {
        ...
        if (parsed_data.is_winner) {
          status_class =
            '<i class="fa fa-fw fa-smile-o"></i> Congrats, you won!';
          bet_class = "success";
        } else {
          status_class =
            '<i class="fa fa-fw fa-frown-o"></i> You lost, try again!';
          bet_class = "danger";
        }
        ...
        var wageredCol =
          '<td class="text-warning"><i class="fa fa-fw fa-database"></i> ' +
          parsed_data.wagered +
          "</td>";
        ...
        return $outcomeStatus
          .addClass("alert alert-info alert-small")
          .html(status_class);
      })
      .fail(function (xhr, status, error) {
        var status_class = "alert-error";
